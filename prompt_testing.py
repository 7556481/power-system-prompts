...
def apply_evidence_based_overrides(
    ...
    technical_claim_support_used = citation_analysis.get("claim_present", False)
    ...
    if has_high_safety_alert and has_unverifiable_citation and technical_claim_support_used:
        final_score = max(final_score, 0.72, float(safety_result.get("risk_score", 0.0)))
        final_level = "High"
        override_reasons.append(
            "Escalated to High because operational safety alerts co-occur with unverifiable citations used to support technical claims."
        )
